...
theoretically. Section~\ref{sec:conclusion} concludes.
...
optimum. Our best submission~(AN45c, 2{,}252~bytes) achieves~79.25\%
on gpt-oss-120b ($n=400$; 95\%~CI: [75.0\%, 82.9\%]), with \textsc{True}
recall of~95.9\% and \textsc{False} recall of~63.4\%. A cross-provider
validation run on OpenRouter/DeepInfra~bf16 ($n=20$) yielded~90--95\%,
consistent with the full-scale result. The token cap finding demonstrates
that Gemma's performance (50\% vs.~85\%) is entirely determined by
provider configuration, not model capability. Model-specific bias
profiles~---~\textsc{True} bias in gpt-oss-120b, instruction-capacity
collapse in Llama, token-gated reasoning in Gemma~---~are stable across
variants and not correctable by prompt engineering alone.
