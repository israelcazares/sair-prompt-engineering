...
\subsection{The Local--Official Inversion}
\label{sec:official-inversion}
...
above~65\% accuracy on both distributions simultaneously.
Section~\ref{sec:official-tradeoff} revisits this exception with the complete leaderboard.
All other
...
our own (AN45c, 2{,}252~bytes, heuristic reasoning guidance) and the
competition winner's (9{,}834~bytes, rule-ordered deterministic
classification; Section~\ref{sec:official-router}). For each submission
this yields $200 \times 3$ run-level outcomes per model per problem
set---7{,}200 outcomes across the scored board's nine model$\times$set
cells, plus 1{,}800 for our submission on the research board.
...
inconsistent verdicts across three identical runs; under the winner's
rule-ordered classifier, only 9\% do. This pattern is consistent with
the interpretation that a rule-ordered procedure constrains the
model's decision process more tightly than reasoning guidance does:
under the former, identical runs converge on identical verdicts; under
the latter, the model resolves the same problem differently run to
run.
